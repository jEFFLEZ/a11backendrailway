...
exports.applyCortexPacket = applyCortexPacket;
...
function saveCache(cache) {
    try {
        if (!fs.existsSync(QFLUSH_DIR))
            fs.mkdirSync(QFLUSH_DIR, { recursive: true });
...
function appendPatchAudit(entry) {
    try {
        ensurePatchesDir();
    ...
    try {
        ...
        for (const k of keys) {
            if (!whitelist.includes(k))
                return { ok: false, reason: `key_not_allowed:${k}` };
        }
        return { ok: true };
    }
    catch (e) {
...
async function applyCortexPacket(packet) {
    try {
...
async function applyNpzGraph(packet) {
    try {
        const payload = packet.payload ?? {};
        const filePath = String(payload?.path ?? process.cwd());
        console.log('[APPLY] NPZ-GRAPH ingest for', filePath);
        const res = await (0, executor_js_1.executeAction)('npz.encode', { path: filePath });
...
async function applySaveState(packet) {
    try {
        if (!fs.existsSync(QFLUSH_DIR))
            fs.mkdirSync(QFLUSH_DIR, { recursive: true });
...
async function applyAutoPatch(packet) {
    try {
        const payload = packet.payload ?? {};
...
async function applyEnableSpyder(packet) {
    try {
...
async function applyCortexRoutes(packet) {
    try {
        const dir = path.join(process.cwd(), '.qflush');
...
async function applyQflushConfigPatch(packet) {
    try {
        const dir = path.join(process.cwd(), '.qflush');
...
async function applyOc8(packet) {
    try {
        const payload = packet.payload ?? {};
...
async function applyQrouter(packet) {
    try {
        const payload = packet.payload ?? {};
...
async function applySpyderSound(packet) {
    try {
        const payload = packet.payload ?? {};
...
async function applyA11Key(packet) {
    try {
        const payload = packet.payload ?? {};
...
async function applyMagic(packet) {
    try {
...
exports.default = { applyCortexPacket };
